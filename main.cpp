...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR nCmdLine, int nCmdShow)
{
	...
	path = path.substr(0, path.size() - 23);
	
	std::string vTemp = path + "Kartrider\\VertexShader.hlsl";
	...
	std::string pTemp = path + "Kartrider\\PixelShader.hlsl";
		...
		if (!gameStart) {
			...
			if (!Collided(trackCourse->GetVertices(), car->GetBoundary())) {
				...
				if (Finished(trackCourse->GetFinishLine(), car->GetBoundary()) && disTraveled > window->GetSize().first) {
					...
					MessageBox(NULL, ss.str().c_str(),"Finished", MB_OK);
					gameStart = false;
				}
				...
				MessageBox(NULL, "", "y u crashin", MB_OK);
				trackCourse = new TrackCourse(graphics, shaderByteCode, shaderByteSize);
				car = new Car(graphics, shaderByteCode, shaderByteSize);
				speed = angle = 0.0f;
				disTraveled = 0;
